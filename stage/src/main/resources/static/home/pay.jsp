...
<script type="text/javascript">
	...
	//点击提交订单
	 $("#J_Go").click(function(){
			...
			reset_form($("#myModal form"));
			   ...
			   $.ajax({
					    ...
					    if(msg.code==200)
				   {
					    	...
					    	setTimeout(function(){
					    		$(location).prop("href","../test/test_main.jsp?action=question");
					    								},3000);
				   }
					   var type = msg.map.list[0].questionType;
					   var question = "" ;
					   switch (type){
  	 ...
  	 $("#check_answer").click(function(){
  		 ...
  		 if(flag==1)
  		{
  		 $.post("../../Order/finishForderAndSorderByForderId","forderId="+$("#forder_id").val()+"&logistics="+$("#logistics_id").val()+"&money="+$("#J_ActualFee").html(),function(data,status){
            if(data.code==100)
         	  $(location).prop('href',"../home/success.jsp?address="+data.map.addressId); 
            else if(data.code==200)
         	  {
            	if(data.map.status == 1){
                	...
                	setTimeout(function(){
                		$(location).prop("href","../test/test_main.jsp?action=address");
    					},3000);
  ...
  //点击创建新地址
	...
	function item_list(/* msg */) {
		...
		$.post("../../Order/CheckForderStatus","id="+$("#forder_id").val(),function(status){
			        if(status.code==100)
			        	{
			        	if(status.map.statue != 0)
			        		{
		...
		$.post("../../Order/selectSorderByForderId","id="+$("#forder_id").val(),function(data,status){
			console.log(data);
			if(data.code==100){
				
			$.each(data.map.list,function(index,item){
				...
				/* 	input4_1.addClass("min am-btn").attr("name", "").attr("type", "button")
					...
					input4_2.addClass("text_box").attr("name", "").attr("type", "text")
							.attr("value", item.number).attr("style", "width:30px;").attr("readonly",true);
				/* 	input4_3.addClass("add am-btn").attr("name", "").attr("type", "button")
					...
					em5.append(parseInt(item.number)*parseInt(product.price));
					  $("#all_moneys").val( $("#all_moneys").val()+parseInt(item.number)*parseInt(product.price));
					//all_money += parseInt(item.number)*parseInt(product.price);
				 all += parseInt(item.number)*parseInt(product.price);
	             ...
	             $.each(list, function(index, addres) {
				   ...
				   if(addres.selected == 1){
	    		        ...
	    		        var  in_i_three = $("<i></i>").addClass("am-icon-edit");
	    		         ...
	    		         var a_one = $("<a></a>").attr("href","../test/test_main.jsp?action=address").append(in_i_three).append("编辑");
	    		        ...
	    		        var a_two = $("<a></a>").attr("href","../test/test_main.jsp?action=address").append(in_i_four).append("删除");
...
<!-- 模态框 -->
 ...
 <input id="all_moneys" type="hidden" value="${param.allmoney} " />
 <input id="forder_id" type="hidden" value=" ${param.forderId }" />
