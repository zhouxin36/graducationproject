	...
	methods: {
		...
		del: function () {
			...
			confirm('确定要删除选中的记录？', function(){
                $.get(baseURL + "category_delete","id="+id, function(r){
                    if (r.code != 200) {
                        alert(r.msg);
                        return;
                    }
		...
		saveOrUpdate: function () {
			...
			var url = vm.app.id == null ? "category_add" : "category_update";
			$.ajax({
				type: "POST",
			    url: baseURL + url,
		...
		getApp: function(id){
			$.get(baseURL + "select_category","id="+id, function(r){
